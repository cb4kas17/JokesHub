...
function EditJoke() {
    ...
    const onUpdateHandler = (e) => {
        ...
        async function updateData() {
            try {
                const response = await axios.put(`http://localhost:4000/api/EditJokes/${jokeID}`, content, {
                    headers: {
                        'x-access-token': localStorage.getItem('token'),
                    },
                });
    ...
    const onDeleteHandler = async (e) => {
        try {
            const response = await axios.delete(`http://localhost:4000/api/EditJokes/${jokeID}`, {
                headers: {
                    'x-access-token': localStorage.getItem('token'),
                },
            });
